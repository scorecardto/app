...
export function averageAssignments(
    ...
    return categories.map((category, i) => {
        ...
        (modifiedAssignments[i] ?? category.assignments).forEach((assignment, i) => {
            if (assignment == null) assignment = category.assignments[i];
...
export function averageGradeCategories(
  categories: GradeCategory[],
) {
  let sum = 0;
  let count = 0;
